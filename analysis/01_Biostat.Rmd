...
Geno.Treatment_Anova=apply(counts,1, function(x){
  res = summary(aov(unlist(x)~samplemeta$corrGenotype_Treatment)) %>% unlist()
  return(res[["Pr(>F)1"]])
})

Geno.Treatment_Anova_fdr = p.adjust(Geno.Treatment_Anova, method = "fdr")

Celltype_Anova=apply(counts,1, function(x){
  res = summary(aov(unlist(x)~samplemeta$Celltype)) %>% unlist()
  return(res[["Pr(>F)1"]])
})

Celltype_Anova_fdr = p.adjust(Celltype_Anova, method = "fdr")

summarytab=cbind(sumstat, Geno.Treatment_Anova, Geno.Treatment_Anova_fdr , sumstatCelltype, Celltype_Anova, Celltype_Anova_fdr)
...
```{r basic QC, fig.height=12, fig.width=12}
...
ann_colors = list(
  ...
  Celltype=cellcol,
  Treatment=strokecol, 
  Age=colorRampPalette(c("skyblue", "dodgerblue4"))(20)
)
...
the logFC should not be interpreted on its own without the specific post hoc tests (see app below to check the individual genes) In any case the value here woudl correspond to the APPPS1+ with Stroke against all others.
