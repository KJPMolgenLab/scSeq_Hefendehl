...
outputdir="/files/scSeq_Hefendehl/output/Res_202602/"
...
```{r load_annotations, cache=T, include=FALSE}
...
```{r load_add_samples 2nd, cache=T, dependson="load_add_meta"}
...
# load first batch 
```{r load_add_samples, cache=T, dependson="load_add_samples 2nd"}
...
NOTE: 
We did in the end not fitler for Afip1 negative cells,
...
e
ggsave(paste0(outputdir, "cluster_Sexgenes_integrated_nofilter.pdf"), c, limitsize = F)
```
...
# reslist <- list()
# for(s in seq(2,5,4)){
#   for (d in seq(0.1,1,5)){
#     tmp <-  samples.integrated.filt %>% 
#       RunUMAP(reduction = "harmony", 
#               spread = s, 
#               min.dist = d,  dims=1:ndims) %>% DimPlot(reduction="umap") + ggtitle(paste0("spread:", s, "dist:", d))
#     reslist <-c(reslist, tmp)
#     
#   }
# }
# 
# ggarrange(plotlist= reslist)
...
```{r, fig.width=8, fig.height=7}
samples.integrated.filt_batch1 <- readRDS("/files/scSeq_Hefendehl/output/SeuratObjectafterProcessing.rds")
...
ggsave(paste0(outputdir, "MGscores_FeatMapsintegrated.pdf"), pcomb)



p2<-ggarrange(plotlist=p2, nrow = 2, ncol = 2, common.legend = T)

p2

ggsave(paste0(outputdir, "MGscores_RidgePlotsintegrated.pdf"), p2)
...
if(F){
  ...
  spatialtypes = scSorter(reads, annodf) #makes the calling 
  ...
  DimPlot(samples.integrated.filt, group.by = "PredType",  reduction = "umap",split.by = "Genotype_Treatment")
  
  samples.integrated.filt.comorbid <- samples.integrated.filt[,metadata$Genotype_Treatment == "APPPS1_Stroke"]
  DimPlot(samples.integrated.filt.comorbid,  reduction = "umap",group.by = "PredType", )
  
  crosstab = table(Condition = samples.integrated.filt$Genotype_Treatment, Spatial_Type = samples.integrated.filt$PredType)
  
  
  ggplot(as.data.frame(crosstab), aes(fill=Spatial_Type, x=Condition, y=Freq)) + geom_bar(position="fill",  color="black",stat="identity")+theme_classic()
  crosstab
  chisq.test(crosstab)
  
  ggplot(as.data.frame(crosstab[1:2,]), aes(fill=Spatial_Type, x=Condition, y=Freq)) + geom_bar(position="fill",  color="black",stat="identity")+theme_classic()
  crosstab[1:2,]
  chisq.test(crosstab[1:2,])
  
  ggplot(as.data.frame(crosstab[3:4,]), aes(fill=Spatial_Type, x=Condition, y=Freq)) + geom_bar(position="fill",  color="black",stat="identity")+theme_classic()
  crosstab[3:4,]
...
p2
ggsave(file=paste0(outputdir, "Microgila_WGCNA_Eigenscore_integrated.pdf"), p2)
...
samples.integrated.filt <- AddModuleScore(object = samples.integrated.filt, assay = "RNA", layer="data",
...
ggsave(paste0(outputdir, "Ridge_Custom_Genes_integrated.pdf"), p)
```
...
ggsave(paste0(outputdir, "Ridge_MGSubtypes_Genes_integrated.pdf"))
