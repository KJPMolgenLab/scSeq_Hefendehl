...
see <https://www.nature.com/articles/s41593-023-01355-y> ; see Fig 1e
...
Panel A — Volcano plot from the bulk integrated DEG (APPPS1+stroke vs. APPPS1 Ctrl). We already have this data in the Excel file. Label and highlight Apoe, C4b, Lipe on the positive side and Tyrobp, Cst7, Ccl6, Siglech on the negative side. This gives the immediate visual punch — the two opposing gene groups are physically separated on the plot, and the significance is explicit.
Panel B — Split dot plot showing the same key genes (two groups: "DAM effectors" and "lipid-metabolic") across all six clusters, split by APPPS1 Ctrl vs. APPPS1+stroke. Dot size = percent expressing, colour = mean expression. This is the critical panel that proves the shift is cross-cluster, not just a Microglia2 effect.


## Addtinal Plot Jasmin 
